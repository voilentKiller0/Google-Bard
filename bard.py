...
if __name__ == "__main__":
    print(
        ...
        For send the message to the bard press/hit enter 2 times
        For exit the conversation type !exit
        For reset the conversation type !reset
        """,
    )
    ...
    parser.add_argument(
        "--quick_chat",
        help="Make true if you want to quick chat",
        type=bool,
    ...
    try:
        while True:
            console.print("You:")
            user_prompt = get_multi_line_input()
            console.print()
            if user_prompt == "!exit":
                break
